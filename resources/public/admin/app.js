...
$('.navbar-search-field').on('input', function () {
    ...
    console.log(search_result_pane);
    ...
    // search
    var match = $('.sidebar-nav .menu').filter(function (idx, elem) {
    ...
    // search not found
    if (match.length == 0) {
        $(search_result_pane).append('<li class="text-muted pl-5 text-left">No search result found.</li>');
        return;
    }
    ...
    match.each(function (idx, elem) {
      ...
      parent = `<small class="d-block text-left">${parent}</small>`
      var item_url = $(elem).attr('href') || $(elem).data('default-url');
      var item_text = $(elem).text().replace(/(\d+)/g, '').trim();
      $(search_result_pane).append(`
        <li>
          ${parent}
          <a href="${item_url}" class="fw-bold text-color--3 d-block text-left">${item_text}</a>
        </li>
      `);
    });
