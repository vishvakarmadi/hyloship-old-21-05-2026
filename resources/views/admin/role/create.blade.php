        ...
        <form action="{{ route('admin.role.store') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <td>
                                        <div class="row text-left">
                                            @foreach($matchedPermissions as $key => $per)
                                                @php $name = strtok($per, ' '); @endphp
                                                <div class="col-md-2 custom-control custom-checkbox">
                                                    <input type="checkbox" class="custom-control-input checkbox-tick" id="permission{{ $key }}" name="permissions[]" value="{{ $key }}">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <button type="submit" class="btn--primary btn h-45 w-100 col-lg-4">@lang('Submit')</button>
                        </div>
                    </div>
                </div>
            </div>
