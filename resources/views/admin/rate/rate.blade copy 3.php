...
@section('admin_content')
    ...
    <div class="col-md-12">
        <form class="product-form" action="{{ route('admin.bulkrate.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-header">
                    ...
                    <a href="{{ asset('public/ratecards.xlsx') }}" download="" class="btn btn-secondary">Download Format</a>
                </div>
    ...
    <div id="main_content">
        ...
        <li id="page2" onclick="change_tab(this.id);"><a href="#about" data-toggle="tab">Ratecard</a></li>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <table class="table table-bordered">
                                ...
                                @php 
                                $rate4= DB::table('ratecards')->where('weight',1)->get();
                                $rate3= DB::table('ratecards')->where('weight',0.5)->get();
                                $i=0;
                                if ($weight > 0.5) {
                                    $w = 0;
                                    $additionalHalfKg = ceil($weight - 0.5);
                                    // foreach ($rate3 as $row) {
                                    //     $weigh = $row->within_city;
                                    //     for ($i = 0.5; $i < $additionalHalfKg; $i++) {
                                    //         $rate4[$w]->within_city += $weigh;
                                    //     }
                                    //     $w++;
                                    // }
                                }
                                    ...
                                    @endphp
                                        <tr>
                                            ...
                                            @if($pick->Zone == 'Z1' && $drop->Zone == 'Z1')
                                                <td>Rs.@if($reverse == 'Reverse') {{ $row->within_city * 1.5 }} @elseif($reverse == 'Reverse' && $weight > 0.5) {{ ($row->within_city+ $rate4[$i]->within_city) * 1.5 }} @elseif($weight > 0.5) {{$rate4[$w]->within_city += $weigh}}  @else {{ $row->within_city }} @endif</td>
                                            @elseif($pick->Zone == 'Z2' && $drop->Zone == 'Z2')
                                                <td>Rs.@if($reverse == 'Reverse') {{ $row->within_state * 1.5 }} @elseif($reverse == 'Reverse' && $weight > 0.5) {{ ($row->within_city+ $rate4[$i]->within_state) * 1.5 }}  @elseif($weight > 0.5) {{$rate4[$w]->within_state += $weigh}}   @else {{ $row->within_state }} @endif</td>
    ...
    <script>
        function calculate() {
            var formData = new FormData(document.getElementById("rateForm"));
    
            var xhr = new XMLHttpRequest();
            xhr.open("POST", "{{ route('admin.rate.calculate') }}", true);
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4) {
                    document.getElementById("page2_desc").innerHTML = xhr.responseText;
                    change_tab('page2');
                }
            };
            xhr.send(formData);
    ...
    <script>
        ...
        //   alert(lastTab);
          if (lastTab) {
        //   alert(lastTab);
            document.getElementById("page2").className = "selected";
          }
