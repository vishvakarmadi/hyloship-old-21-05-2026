        ...
        <div class="label-4x6">
            ...
            <div style="display: flex; gap: 8px; margin-bottom: 5px;">
               ...
               @if (!($ls && $ls->hide_rto_address) && (isset($order->rto_warehouse) || isset($order->warehouse)))
                    @php
                        $w = ($order->return_warehouse_id ? \App\Models\Admin\Warehouse::find($order->return_warehouse_id) : null) ?? $order->warehouse;
                    @endphp
                    <div class="return-box" style="flex: 1; border: 1.5px solid #000; padding: 4px 6px; font-size: 8px;">
                        <div class="label-tag">RTO Address</div>
                        @if(!($ls && $ls->hide_rto_name))<b>{{ $w->name }}</b><br>@endif
                        @if(!($ls && $ls->hide_rto_contact_name) && isset($w->contact_person))<span>Attn: {{ $w->contact_person }}</span><br>@endif
                        {{ $w->address }}@if(!empty($w->city)), {{ $w->city }}@endif @if(!empty($w->pincode)) – {{ $w->pincode }}@endif
                        @if($w->mobile && !($ls && $ls->hide_rto_mobile))<br>Ph: {{ $w->mobile }}@endif
                    </div>
                @endif
            </div>
            ...
            <div class="footer-note">
               @php
    $f_mobile = $ls->mobile ?? ($general_setting->phone ?? null);
    $f_email  = $ls->email ?? ($general_setting->email ?? null);

    $show_mobile = $f_mobile && !($ls && $ls->hide_pickup_mobile);
@endphp

@if($show_mobile || $f_email)
    <b>Contact:</b>
