...
<body>
    ...
    @foreach ($orders as $order)
        <div class="label-4x6">
            ...
            <div class="bc-row" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
                ...
                @if(!($ls && $ls->logo_hidden))
                    ...
                    <div style="flex: 0 0 auto; text-align: right;">
                        <img src="{{ asset('public/uploads/' . $logo_path) }}" style="max-height: 40px; max-width: 120px; object-fit: contain;">
                    </div>
                    @endif
                @endif
            </div>
            ...
            @if($order->tracking_info)
            <div class="bc-row" style="margin-top: 5px; margin-bottom: 10px;">
            ...
            <div style="display: flex; gap: 8px; margin-bottom: 5px;">
                @if (!($ls && $ls->hide_pickup_address) && isset($order->warehouse) && !empty($order->warehouse))
                    ...
                    @endphp
                    <div class="return-box" style="flex: 1; border: 1.5px solid #000; padding: 4px 6px; font-size: 8px;">
                        ...
                        @endif
                        {{ $w->address }}@if(!empty($w->city)), {{ $w->city }}@endif @if(!empty($w->pincode)) – {{ $w->pincode }}@endif
                ...
                @if (!($ls && $ls->hide_rto_address))
                    @php
                        $rw = ($order->return_warehouse_id ? \App\Models\Admin\Warehouse::find($order->return_warehouse_id) : null) ?? $order->warehouse;
                    @endphp
                    @if($rw)
                    <div class="return-box" style="flex: 1; border: 1.5px solid #000; padding: 4px 6px; font-size: 8px;">
                        <div class="label-tag">RTO Address</div>
                        @if(!($ls && $ls->hide_rto_name))<b>{{ $rw->name }}</b><br>@endif
                        @if(!($ls && $ls->hide_rto_contact_name) && isset($rw->contact_person))<span>Attn: {{ $rw->contact_person }}</span><br>@endif
                        {{ $rw->address }}@if(!empty($rw->city)), {{ $rw->city }}@endif @if(!empty($rw->pincode)) – {{ $rw->pincode }}@endif
                        @if($rw->mobile && !($ls && $ls->hide_rto_mobile))<br>Ph: {{ $rw->mobile }}@endif
                    </div>
                    @endif
                @endif
            </div>
            ...
            <div class="hdr-row">
                ...
                <div class="hdr-right">
                    ...
                    @if(!($ls && $ls->hide_gst_number) && $kyc_gst)
                        <div style="font-size: 8px; margin-top: 2px;">GSTIN: {{ $kyc_gst }}</div>
                    @endif
                </div>
            </div>
            ...
            @php
                ...
                $total_item_discount = $order->detail->sum(function($d) {
                    return ($d->discount_type == 'p') ? ($d->price * $d->qty * $d->discount) / 100 : (float)$d->discount;
                });
            @endphp
            <table class="products">
                <thead>
                    <tr>
                        ...
                        @if(!($ls && $ls->hide_discount_amount)) <th style="width:40px">Disc</th> @endif
                        @if($ls && $ls->show_hsn) <th style="width:50px">HSN</th> @endif
                        @if($ls && $ls->show_gst) <th style="width:50px">Tax</th> @endif
                ...
                <tbody>
                    @foreach ($order->detail as $d)
                        @php
                            $row_discount = ($d->discount_type == 'p') ? ($d->price * $d->qty * $d->discount) / 100 : (float)$d->discount;
                        @endphp
                        <tr>
                            <td>
                                {{ $d->name }}
                            ...
                            @if(!($ls && $ls->hide_discount_amount)) <td style="text-align:right">₹{{ number_format($row_discount, 0) }}</td> @endif
                    ...
                    @if(!($ls && $ls->hide_total_amount))
                        ...
                        @php
                            $discount = (float)$order->discount + $total_item_discount;
                        @endphp
                        @if (!($ls && $ls->hide_discount_amount) && $discount > 0)
                            <tr>
