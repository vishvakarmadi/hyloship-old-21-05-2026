...
class ApiOrderController extends Controller
{
    ...
    { 
        ...
        if (!$admin) {
            ...
            return response()->json(['error' => true, 'message' =>  'Token has expired. Please refresh your token.'], 401);
        }
        ...
        if($request->total ==0 || $request->total ==''){
            return response()->json(['error' => true, 'message' => 'Order value can not be 0'], 400);
        }
        ...
        if($order){
            ...
            return response()->json(['error' => true, 'message' => 'Wrong Data'], 400);
        }
        }else{
            return response()->json(['error' => true, 'message' => 'Product Data Mismatch'], 400);
        }
    ...
    public function orderawb(Request $request){
        ...
        if($request->warehouse_id ==0 || $request->return_warehouse_id ==0){
            return response()->json(['error' => true, 'message' =>  'Warehouse id cannot be 0'], 422);
        }
        }else{
            return response()->json(['error' => true, 'message' =>  'Warehouse id not found'], 422);
        }
        $orderawb = Api::ordercreateawb($request, $user_id);
        return response()->json([$orderawb[0] => true, 'message' => $orderawb[1]], 200);
    ...
    public function updateorder(Request $request, $id){
       ...
       $user_id=$admin->id;
       $response = Api::orderupdate($request,$user_id,$id);
       return $response;
    }
    ...
    //   dd($id);
        $response=Api::orderdelete($id,$user_id);
        // dd($response);
    ...
    public function cancelorder($id){
        ...
        if (!$admin) {
            ...
            if($getuser->user != $admin->id){
                return response()->json(['error' => true, 'message' => 'No Access for this order'], 400);
            }
            if($getuser->status =='4'){
                return response()->json(['error' => true, 'message' => 'Already cancelled'], 400);
            }
            if(!in_array($getuser->status, array('1','2','12'))){
                return response()->json(['error' => true, 'message' => 'Cannot cancel processed order,please contact admin'], 400);
            }
            $response = Api::ordercancel($id);
            return response()->json([$response[0] => true, 'message' => $response[1]], 200);
        }
    }
    ...
    public function trackorder($id){
        ...
        if ($admin->expired_on <= now()) {
            ...
            if($order){
                ...
                if(!$tracking_info){
                    return response()->json(['error' => true, 'message' =>  'Awb not generated'], 400);
                }
...
//                    echo '<pre>';print_R($status_progressarray);die;
                ...
                return response()->json(['success' => true, 'shipment_history' => $progress], 200);
            }else{
                return response()->json(['error' => true, 'message' => 'No Access for this order'], 400);
            }
        }
